 ...
 */
import type { Patient, PaginatedResponse, AestheticPlan } from '~~/app/types'
...
const { data } = await useAsyncData(
  ...
  async () => {
    ...
    try {
      const res = await api.get<PaginatedResponse<AestheticPlan>>(
...
<template>
  <UAlert
    ...
    :actions="[{ label: t('treatmentPlans.newBudgetHint.cta'), to: `/aesthetic-plans/${plan.id}`, color: 'warning', variant: 'solid' }]"
  />
</template>
