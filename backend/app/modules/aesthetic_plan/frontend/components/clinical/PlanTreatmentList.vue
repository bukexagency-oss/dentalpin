...
import type { AestheticPlanItem } from '~~/app/types'
...
const itemToComplete = ref<AestheticPlanItem | null>(null)

function openConfirmModal(item: AestheticPlanItem) {
...
const localPending = ref<AestheticPlanItem[]>([])
...
function getItemName(item: AestheticPlanItem): string {
  ...
  if (clinicalType) {
    const key = `treatments.types.${clinicalType}`
...
function itemTeeth(item: AestheticPlanItem): number[] {
  return (item.treatment?.teeth ?? []).map(t => t.tooth_number)
}

function itemSurfaces(item: AestheticPlanItem): string[] {
  const first = item.treatment?.teeth?.[0]
  return (first?.surfaces as string[] | undefined) ?? []
}

function formatToothInfo(item: AestheticPlanItem): string {
...
function hasToothInfo(item: AestheticPlanItem): boolean {
  return itemTeeth(item).length > 0
}

/** Catalog price to strike through when the accepted quote lowered it. */
function discountedFrom(item: AestheticPlanItem): number | undefined {
...
function isMultiSession(item: AestheticPlanItem): boolean {
  return (item.sessions?.length ?? 0) > 1
}

function sessionProgress(item: AestheticPlanItem): { done: number, total: number } {
