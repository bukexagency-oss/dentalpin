...
const props = defineProps<{
  modelValue: boolean
  plan?: AestheticPlan | null
  patientId: string
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: boolean): void
  (e: 'saved', plan: AestheticPlan): void
}>()

const { t } = useI18n()
const auth = useAuth()
const { createPlan, updatePlan, loading } = useAestheticPlans()
...
const form = ref<AestheticPlanCreate | AestheticPlanUpdate>({
...
watch(
  ...
  async (open) => {
    if (open) {
      ...
      if (props.plan) {
        const planDetail = props.plan as AestheticPlan & { diagnosis_notes?: string, internal_notes?: string }
        form.value = {
...
function countMatchingPendingItems(): number {
  const detail = props.plan as (AestheticPlan & { items?: AestheticPlanItem[] }) | null | undefined
...
async function performUpdate(reassignPending: boolean): Promise<void> {
  const payload: AestheticPlanUpdate = {
    ...(form.value as AestheticPlanUpdate),
...
async function handleSubmit() {
  ...
  const result = await createPlan(form.value as AestheticPlanCreate)
  if (result) {
    emit('saved', result)
    isOpen.value = false
  }
