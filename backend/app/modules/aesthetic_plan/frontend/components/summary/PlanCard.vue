 ...
 * click vs. the old IA.
 */
import type { PatientExtended, AestheticPlan, PaginatedResponse } from '~~/app/types'
...
const { data, status } = await useAsyncData(
  ...
  async () => {
    try {
      return await api.get<PaginatedResponse<AestheticPlan>>(
        `/api/v1/aesthetic_plan/aesthetic-plans?patient_id=${patientId.value}`
      )
    } catch {
...
const activePlan = computed<AestheticPlan | null>(() => {
...
const planHref = computed(() =>
  activePlan.value
    ? `/aesthetic-plans/${activePlan.value.id}?from=patient&patientId=${patientId.value}`
