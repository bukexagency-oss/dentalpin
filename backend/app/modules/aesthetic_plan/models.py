...
class AestheticPlan(Base, TimestampMixin):
...
class AestheticPlanItem(Base, TimestampMixin):
    ...
    __tablename__ = "aesthetic_plan_items"
    ...
    treatment_plan: Mapped["AestheticPlan"] = relationship(back_populates="items")
    treatment: Mapped["Treatment"] = relationship()
    completer: Mapped["User | None"] = relationship(foreign_keys=[completed_by])
    assigned_professional: Mapped["User | None"] = relationship(
        foreign_keys=[assigned_professional_id]
    )
    sessions: Mapped[list["AestheticPlanItemSession"]] = relationship(
        back_populates="plan_item",
        cascade="all, delete-orphan",
        order_by="AestheticPlanItemSession.sequence",
    )
...
class AestheticPlanItemSession(Base, TimestampMixin):
    """One billable / executable step of a ``AestheticPlanItem``.
    ...
    __tablename__ = "aesthetic_plan_item_sessions"

    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    plan_item_id: Mapped[UUID] = mapped_column(
        ForeignKey("aesthetic_plan_items.id", ondelete="CASCADE"), index=True
    )
    ...
    # Relationships
    plan_item: Mapped["AestheticPlanItem"] = relationship(back_populates="sessions")
    completer: Mapped["User | None"] = relationship(foreign_keys=[completed_by])
