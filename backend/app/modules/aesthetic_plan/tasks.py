...
async def _close_for_clinic(
    ...
    from .service import AestheticPlanService  # avoid circular at import-time
    ...
    async with sem, async_session_maker() as db:
        try:
            ...
            for row in rows:
                plan = await AestheticPlanService.get(db, clinic_id, row.id)
                if not plan:
                    continue
                try:
                    await AestheticPlanService.close(
                        db,
                        clinic_id,
                        plan.id,
                        plan.created_by,
