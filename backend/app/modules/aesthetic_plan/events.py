...
from .models import AestheticPlanItem, AestheticPlanItemSession
...
async def on_appointment_completed(data: dict[str, Any], *, db: AsyncSession) -> None:
    ...
    from .service import AestheticPlanService
    ...
    for apt_treatment in completed_treatments:
        ...
        item_result = await db.execute(
            select(AestheticPlanItem).where(
                AestheticPlanItem.id == apt_treatment.planned_treatment_item_id,
                AestheticPlanItem.clinic_id == UUID(clinic_id),
            )
        )
        ...
        await AestheticPlanService._check_and_complete_plan(
...
async def on_budget_accepted(data: dict[str, Any], *, db: AsyncSession) -> None:
    ...
    Idempotent: ``AestheticPlanService.accept_from_budget`` is a no-op
    ...
    await AestheticPlanService.accept_from_budget(db, UUID(clinic_id), UUID(plan_id), line_amounts)
...
async def on_budget_rejected(data: dict[str, Any], *, db: AsyncSession) -> None:
    ...
    await AestheticPlanService.reject_from_budget(
...
async def on_budget_renegotiated(data: dict[str, Any], *, db: AsyncSession) -> None:
    ...
    await AestheticPlanService.reopen_from_budget(db, UUID(clinic_id), UUID(plan_id))
...
async def on_budget_superseded(data: dict[str, Any], *, db: AsyncSession) -> None:
    ...
    Transactional: ``aesthetic_plans.budget_id`` is an FK to the new budget
    ...
    plan = await AestheticPlanService.get(db, UUID(clinic_id), UUID(plan_id))
...
async def on_treatment_performed(data: dict[str, Any], *, db: AsyncSession) -> None:
    """Handle treatment performed from 
    ...
    ``AestheticPlanService.complete_item``, which has already flushed its own
    ...
    result = await db.execute(
        ...
        .where(
            AestheticPlanItem.treatment_id == UUID(treatment_id),
            AestheticPlanItem.clinic_id == UUID(clinic_id),
    ...
    # second time.
    await db.execute(
        update(AestheticPlanItemSession)
        .where(
            AestheticPlanItemSession.plan_item_id == item.id,
            AestheticPlanItemSession.status == "pending",
        )
    ...
    await AestheticPlanService._check_and_complete_plan(db, UUID(clinic_id), item.treatment_plan_id)
