...
- fix(#184): the layer type-checks clean under `nuxt typecheck`. Real
  bugs behind the type errors: `AestheticPlanStatus` now matches the API
  (`pending`/`closed` instead of the phantom `cancelled`), toasts use
  Nuxt UI v4 semantic colours (`error`/`success` — they rendered
  uncoloured), `AestheticPlanResponse` exposes `closure_reason` /
...
- chore(ui): removed the unused `AestheticPlanDetail.vue` component.
...
- fix(billing): multi-session items no longer double-charge on finalization.
  ...
  `treatment.performed` event carried the full item price —
...
- fix(lock): a **draft** budget no longer locks the plan — the lock now
  ...
  `AestheticPlanDetail.canEditItems`). Users had to accept the quote
...
- feat(ux): ``PlanDetailView`` wires the new ``PlanNotesTimeline``
  ...
  matching tooth/arch on the  Treatment-source notes resolve
  via ``itemByTreatmentId``; visit-source via ``itemByPlanItemId``;
  plan-level notes no-op.
- feat(sessions): plan items now own 1..N ``AestheticPlanItemSession``
  ...
  next pending session. Migration ``ae_0006`` creates the new table and
...
- fix(events): ``on_treatment_performed`` handler uses
  ...
  on ``PATCH /aesthetic-plans/{id}/items/{id}/complete``: under the
  async-first event bus (sprint 3) the parent transaction held the
  row lock on ``aesthetic_plan_items`` and the handler — running
...
- feat(plans): per-item assigned professional. `AestheticPlanItem` gains
  ...
  `reassign_pending_items` flag on `AestheticPlanUpdate` — only pending items
  still pointing at the previous plan doctor are reassigned; explicit
  overrides and completed items are left alone. Migration `ae_0005` backfills
...
- perf(list): collapse the duplicated ``items → treatment`` eager-load
  chain in ``AestheticPlanService.list`` into a single chain that
...
- **0.2.0 (issue #55)** — `TreatmentMedia` model + `treatment_media`
  table dropped (migration `ae_0004`, depends on `med_0002`). Existing
  ...
  `POST/DELETE /aesthetic-plans/items/{id}/media` endpoints are gone
...
- Unified `Planes de tratamiento` and `Bandeja de planes` into a single
  page at `/aesthetic-plans` with six tabs (5 pipeline workflow tabs +
  `Listado`). The dedicated `/aesthetic-plans/pipeline` route and the
...
- Legacy unlock flow (`POST /aesthetic-plans/{id}/unlock`,
  `AestheticPlanService.unlock`, `treatment_plan.unlocked` event,
...
- Page `/aesthetic-plans/pipeline` (bandeja de planes) with five
...
- `useAestheticPlans` gains `confirmPlan`, `reopenPlan`, `closePlan`,
...
- New endpoints:
  - `POST /aesthetic-plans/{id}/{confirm,reopen,close,reactivate}`
  - `POST /aesthetic-plans/{id}/contact-log`
  - `GET  /aesthetic-plans/pipeline` (5-tab cross-module bandeja).
