...
class AestheticPlanCreate(BaseModel):
...
class AestheticPlanUpdate(BaseModel):
...
class AestheticPlanStatusUpdate(BaseModel):
    """Change plan status."""
...
class ClosePlanRequest(BaseModel):
    """Body for ``POST /aesthetic-plans/{id}/close``."""
...
class ContactLogRequest(BaseModel):
    """Body for ``POST /aesthetic-plans/{id}/contact-log``.
...
class AestheticPlanResponse(BaseModel):
...
class AestheticPlanDetailResponse(AestheticPlanResponse):
    """Detailed response with nested items."""

    diagnosis_notes: str | None = None
    internal_notes: str | None = None
    items: list["AestheticPlanItemResponse"] = []
...
class AestheticPlanItemCreate(BaseModel):
...
class AestheticPlanItemUpdate(BaseModel):
...
class AestheticPlanItemResponse(BaseModel):
...
# instead of the previous ``POST /aesthetic-plans/items/{id}/media``.
#
# Clinical-note schemas moved to the ``clinical_notes`` module — see issue #60.

AestheticPlanDetailResponse.model_rebuild()
