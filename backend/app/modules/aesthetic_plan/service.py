...
def _treatment_loader() -> selectinload:
    ...
    return selectinload(AestheticPlanItem.treatment).options(
...
def _item_with_sessions_loader() -> list:
    ...
    return [
        _treatment_loader(),
        selectinload(AestheticPlanItem.sessions),
    ]


def _snapshot_sessions_from_catalog(
    item: AestheticPlanItem,
    treatment: Treatment,
) -> list[AestheticPlanItemSession]:
    ...
    rows: list[AestheticPlanItemSession] = []
...
# Allowed status transitions for ``AestheticPlan``. See ADR 0006 and
...
def _is_plan_locked(plan: AestheticPlan) -> bool:
...
class AestheticPlanService:
    ...
    async def generate_plan_number(db: AsyncSession, clinic_id: UUID) -> str:
        ...
        result = await db.execute(
            select(func.count(AestheticPlan.id)).where(
                AestheticPlan.clinic_id == clinic_id,
                AestheticPlan.plan_number.like(f"PLAN-{year}-%"),
            )
        )
    ...
    async def list(
        ...
        if patient_id:
            base_where.append(AestheticPlan.patient_id == patient_id)

        if status:
            statuses = [status] if isinstance(status, str) else list(status)
            if len(statuses) == 1:
                base_where.append(AestheticPlan.status == statuses[0])
            else:
                base_where.append(AestheticPlan.status.in_(statuses))

        # Count
        count_result = await db.execute(select(func.count(AestheticPlan.id)).where(*base_where))
        ...
        query = (
            select(AestheticPlan)
            .where(*base_where)
            .options(
                selectinload(AestheticPlan.patient),
                selectinload(AestheticPlan.budget),
                selectinload(AestheticPlan.items)
                .selectinload(AestheticPlanItem.treatment)
                .options(
                    selectinload(Treatment.teeth),
                    selectinload(Treatment.catalog_item),
                ),
                selectinload(AestheticPlan.items).selectinload(AestheticPlanItem.sessions),
            )
            .order_by(AestheticPlan.created_at.desc())
            .offset(offset)
            .limit(page_size)
        )
    ...
    async def create(
        ...
        plan_number = await AestheticPlanService.generate_plan_number(db, clinic_id)
    ...
    async def update(
        ...
        plan = await AestheticPlanService.get(db, clinic_id, plan_id)
        if not plan:
            return None

        # Pull the cascade flag out before generic setattr — it is a
        # write-only directive, not a column on AestheticPlan.
        ...
        if (
            ...
            await db.execute(
                update(AestheticPlanItem)
                .where(
                    AestheticPlanItem.treatment_plan_id == plan_id,
                    AestheticPlanItem.clinic_id == clinic_id,
                    AestheticPlanItem.status == "pending",
                    AestheticPlanItem.assigned_professional_id == old_professional_id,
                )
    ...
    async def update_status(
        ...
        if new_status in ("closed", "archived"):
            await AestheticPlanService._cleanup_orphan_planned_treatments(
                db, clinic_id, plan, user_id
            )

        # Publish event
        await event_bus.publish(
            "aesthetic_plan.status_changed",
            {
                "plan_id": str(plan.id),
    ...
    async def delete(
        ...
        plan are soft-deleted so they disappear from the  `performed`
        ...
        await AestheticPlanService._cleanup_orphan_planned_treatments(db, clinic_id, plan, user_id)
    ...
    async def _cleanup_orphan_planned_treatments(
        ...
        *other* non-terminal plan still references it. If not, and the Treatment
        ...
        """
        from app.modules.service import TreatmentService
        ...
        for treatment_id in treatment_ids:
            ...
            result = await db.execute(
                select(func.count(AestheticPlanItem.id))
                .join(
                    AestheticPlan,
                    AestheticPlan.id == AestheticPlanItem.treatment_plan_id,
                )
                .where(
                    AestheticPlanItem.treatment_id == treatment_id,
                    AestheticPlanItem.clinic_id == clinic_id,
                    AestheticPlanItem.treatment_plan_id != plan.id,
                    AestheticPlan.deleted_at.is_(None),
                    AestheticPlan.status.notin_(("archived", "cancelled")),
                )
            )
    ...
    async def add_item(
        ...
        if sequence_order is None:
            result = await db.execute(
                select(func.max(AestheticPlanItem.sequence_order)).where(
                    AestheticPlanItem.treatment_plan_id == plan_id
                )
            )
        ...
        reloaded = await db.execute(
            select(AestheticPlanItem)
            .options(
                selectinload(AestheticPlanItem.treatment).selectinload(Treatment.teeth),
                selectinload(AestheticPlanItem.treatment).selectinload(Treatment.catalog_item),
                selectinload(AestheticPlanItem.sessions),
            )
            .where(AestheticPlanItem.id == item.id)
        )
        item = reloaded.scalar_one()

        # Snapshot payload — see ADR 0003. The budget module subscribes
        # without importing aesthetic_plan/odontogram models.
        ...
        await event_bus.publish(
            "aesthetic_plan.treatment_added",
            {
                "plan_id": str(plan_id),
                "item_id": str(item.id),
    ...
    async def reorder_items(
        ...
        plan_q = await db.execute(
            ...
            .options(selectinload(AestheticPlan.budget))
        )
        ...
        items_q = await db.execute(
            select(AestheticPlanItem).where(
                AestheticPlanItem.treatment_plan_id == plan_id,
                AestheticPlanItem.clinic_id == clinic_id,
            )
        )
    ...
    async def remove_item(
        ...
        Orphan rule: if the removed item was the last active AestheticPlanItem
        ...
        other_refs = await db.execute(
            select(func.count(AestheticPlanItem.id)).where(
                AestheticPlanItem.treatment_id == treatment_id,
                AestheticPlanItem.clinic_id == clinic_id,
            )
        )
        ...
        await event_bus.publish(
            "aesthetic_plan.treatment_removed",
            {
                "plan_id": str(plan_id),
                "item_id": str(item_id),
                "treatment_id": str(treatment_id),
    ...
    async def complete_session(
        ...
        """
        item = await AestheticPlanService._load_item_with_sessions(db, clinic_id, plan_id, item_id)
        if not item:
            return None
        ...
        plan = await db.get(AestheticPlan, plan_id)
        ...
        if all_terminal and any_completed and item.status != "completed":
            await AestheticPlanService._finalize_item(
                db,
                clinic_id=clinic_id,
                plan_id=plan_id,
                item=item,
    ...
    async def _finalize_item(
        ...
        *,
        ...
        await event_bus.publish(
            "aesthetic_plan.treatment_completed",
            {
                "plan_id": str(plan_id),
                "item_id": str(item.id),
                "treatment_id": str(item.treatment_id),
        ...
        await AestheticPlanService._check_and_complete_plan(db, clinic_id, plan_id)
    ...
    async def complete_item(
        ...
        Items always have ≥1 session post-migration (ae_0006 backfill).
        ...
        return await AestheticPlanService.complete_session(
            db,
            clinic_id=clinic_id,
            plan_id=plan_id,
            item_id=item_id,
    ...
    async def link_budget(
        ...
        # ``aesthetic_plans.budget_id`` is unique — fail clean instead of a 500.
        other = await db.scalar(
            select(AestheticPlan.id).where(
                AestheticPlan.budget_id == budget_id,
                AestheticPlan.id != plan_id,
            )
        )
    ...
    async def request_budget_sync(
        ...
        plan_items = await db.execute(
            select(AestheticPlanItem)
            .options(selectinload(AestheticPlanItem.treatment).selectinload(Treatment.teeth))
            .where(
                AestheticPlanItem.treatment_plan_id == plan_id,
                AestheticPlanItem.clinic_id == clinic_id,
            )
        ...
        await event_bus.publish(
            "aesthetic_plan.budget_sync_requested",
            {
                "plan_id": str(plan_id),
    ...
    @staticmethod
    def _build_plan_snapshot(plan: AestheticPlan, patient: Patient | None) -> dict:
    ...
    async def confirm(
        ...
        snapshot = AestheticPlanService._build_plan_snapshot(plan, patient)
        ...
        # allowed (see aesthetic_plan/CLAUDE.md "Plan→budget direct
    ...
    @staticmethod
    def _reprice_sessions(plan: AestheticPlan, line_amounts: dict[UUID, Decimal]) -> None:
    ...
    async def accept_from_budget(
        ...
        if plan.status == "closed" and plan.closure_reason == "rejected_by_patient":
            ...
            AestheticPlanService._reprice_sessions(plan, line_amounts or {})
            await db.flush()
